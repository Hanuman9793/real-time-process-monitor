...
const CONFIG = {
  ...
  readOnlyToken: 'readonly-' + Math.random().toString(36).substr(2, 9),
...
function authMiddleware(req, res, next) {
  ...
  if (token === CONFIG.password || token === CONFIG.readOnlyToken) return next();
  return res.status(401).json({ success: false, error: 'Unauthorized' });
}

function readOnlyCheck(req, res, next) {
  if (!CONFIG.password) return next();
  const token = req.headers['x-pulse-token'] || req.query.token;
  if (token === CONFIG.readOnlyToken) {
    return res.status(403).json({ success: false, error: 'Read-only mode' });
  }
  next();
}
...
async function getStats() {
  try {
    const [cpuLoad, mem, disk, net, time, cpuTemp, graphics, battery] = await Promise.all([
      si.currentLoad(),
      si.mem(),
      si.fsSize(),
      si.networkStats(),
...
setInterval(async () => {
  ...
  if (stats) {
    // Store in history
    metricsHistory.push({ ts: Date.now(), ...stats });
...
app.get('/system', async (req, res) => {
  ...
  res.json({
    ...
    readOnlyToken: CONFIG.password ? CONFIG.readOnlyToken : null
  });
});

// Kill process
app.post('/kill', authMiddleware, readOnlyCheck, async (req, res) => {
  const { pid } = req.body;
...
app.post('/renice', authMiddleware, readOnlyCheck, async (req, res) => {
...
// Suspend process
app.post('/suspend', authMiddleware, readOnlyCheck, async (req, res) => {
  const { pid } = req.body;
  if (!pid) return res.status(400).json({ success: false, error: 'PID required' });
  try {
...
// Resume process
app.post('/resume', authMiddleware, readOnlyCheck, async (req, res) => {
  const { pid } = req.body;
  if (!pid) return res.status(400).json({ success: false, error: 'PID required' });
  try {
...
// Batch kill
app.post('/kill-batch', authMiddleware, readOnlyCheck, async (req, res) => {
  const { pids } = req.body;
...
app.post('/auth', (req, res) => {
  ...
  if (password === CONFIG.readOnlyToken) return res.json({ success: true, readOnly: true, token: CONFIG.readOnlyToken });
...
server.listen(PORT, () => {
  ...
  if (CONFIG.password) {
    ...
    console.log(`Read-only token: ${CONFIG.readOnlyToken}`);
  }
});